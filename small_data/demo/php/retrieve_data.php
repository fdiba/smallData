<?php
	...
	function retrieveAllCompositionsFrom03($aId){
		...
		list($conc, $fest) = anneesDesOeuvres($dbh, $aId);
	...
	function anneesDesOeuvres($dbh, $aId){
		...
		$sthC = $dbh->prepare('SELECT mc.id_music, mc.annee, s.code
								...
								INNER JOIN imeb_source_secondaire s
									ON s.id = mc.id_source_secondaire
								WHERE m.id_artist = ?');
		$sthC->execute(array((int)$aId));
		...
		$sthF = $dbh->prepare('SELECT mf.id_music, mf.annee, s.code
								...
								INNER JOIN imeb_source_secondaire s
									ON s.id = mf.id_source_secondaire
								WHERE m.id_artist = ?');
		$sthF->execute(array((int)$aId));
		$sthF->setFetchMode(PDO::FETCH_ASSOC);
		while($r = $sthF->fetch()){
	...
	function retrieveAllCompositionsFrom($aId){
		...
		while($row = $sth->fetch()) {
			...
			$str_all .= $row['id'] . "%" . $row['title'] . "%" . $row['duration']
					 . "%" . $row['misam'] . "%" . $row['annee_composition']
					 . "%" . $annees . "%" . $codes;
		}
