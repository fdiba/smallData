...
Particle.prototype.update = function(index, particles){
	...
	this.degagerChevauchement(index, particles);
    ...
    var maxSpeed = this.open ? .6 : (this.ids.length>1 ? 2. : 2.8);
...
Particle.prototype.mergeNodesAndFindTarget = function(index, particles){
	...
	var t = this.seekMergeTarget(index, particles, cand);
	if(t===-2) return;
	if(t<0 && cand){
		t = this.seekMergeTarget(index, particles, null);
		if(t===-2) return;
	}

	if(t>=0){
		this.getCloserFrom(particles[t]); this.mTX=particles[t].x; this.mTY=particles[t].y; this.mHas=true;
	}
...
Particle.prototype.seekMergeTarget = function(index, particles, cand){
	...
	for (var c=0; c<N; c++) {
		...
		if((distance<minDistance && this.ids.length >= p.ids.length) || engulfed){
...
Particle.prototype.avoidGroupsAhead = function(index, particles){
	...
	for (var c=0; c<N; c++) {
		...
		var push = proximity*align*AVOID_STRENGTH*this.scale*this.ids.length*yieldFA*SMA_MOTION;
