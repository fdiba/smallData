...
- **Overview** — an interactive index of the 2,590 identified participants. Each composer is a slate anchor square followed by one coloured square per participation in the competitions. The colour of a participation square is its **edition year**, on a sequential ramp running from sea green (1973) through blue to pale amethyst (2009): it is the *lightness* that carries the order, and a key sits at the top right of the grid. The grid can be reordered three ways — by first entry (the corpus then reads as a staircase), by number of archived works, or alphabetically — and filtered by name or by number of archived records, following Ben Shneiderman's mantra: *overview first, zoom and filter, then details-on-demand*. Selecting a composer opens four folded boxes on the right, the first of which lists their edition years, each marked with what is known of that year — an attested entry to the competition, an entry *and* a work programmed at the Synthèse festival, the festival alone, or a participation recorded with no document attached — and names on hover the record the claim rests on.
...
Every page — both the `small_data/` application and the `imeb/` companion pages — reads from the MySQL database through the PHP endpoints located in each folder.
